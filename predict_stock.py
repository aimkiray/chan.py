...
def run_analysis(code="sz.002701", name="ORG Technology"):
    print(f"Initializing Chan Theory Analysis for {name} ({code})...")
    ...
    if last_snapshot:
        ...
        if latest_bsp_list:
            ...
            print(f"Last Signal Date: {latest_bsp.klu.time} ({days_diff:.1f} days ago)")
...
if __name__ == "__main__":
    if len(sys.argv) > 1:
        ...
        if not code_arg.startswith("sh.") and not code_arg.startswith("sz."):
            if code_arg.startswith("6"):
                code_arg = "sh." + code_arg
            else:
                code_arg = "sz." + code_arg
        run_analysis(code=code_arg, name="Stock " + code_arg)
    else:
        # Default fallback
        run_analysis(code="sz.002701", name="ORG Technology")
